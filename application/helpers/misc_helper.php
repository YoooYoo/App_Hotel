...
// returns true or false for multiple currencies usage
		...
		function pt_check_wishlist($user, $itemid) {
				$CI = get_instance();
		...
		function pt_create_stars($totalstars) {
		  $icon = "";
				for ($stars = 1; $stars <= $totalstars; $stars++) {
						$icon .= PT_STARS_ICON;
				}
                return $icon;
		}
...
// Set map's center position by latitude and longitude coordinates.
                ...
                $map->addMarker($lati, $long, array('title' => urldecode($title), 'defColor' => '#FA6D6D', 'defSymbol' => '', 'html' => '<div style="width:100%;height:120px;"><a href="' . $link . '"><div><img src="' . $img . '" width="' . $imgwidth . '" height="' . $imgheight . '" /></div><b>' . urldecode($title) . '</b></a></div>', 'infoCloseOthers' => true));
				$map->setZoom(14);
// Display the map.
				$map->show();
		}
...
}if(!function_exists('getExtraTitle')){
	function getExtraTitle($extra){
					...
					$CI->db->select('extras_title,extras_desc');
					$CI->db->where('extras_id',$extra);
					$re = $CI->db->get('pt_extras')->result();
					if ($language == DEFLANG) {
					$result = $re[0]->extras_title;
					}else {
