...
// $this->data['modModel'] = $this->modules_model;
    ...
    public function get_all_pending_videos($module)
    {
        if ($module == "hotels") {
            ...
            $this->db->where('vid_approved', '0');
            return $this->db->get('pt_tour_videos')->result();
        }
    }
    ...
    function accept_single_video()
    {
        ...
        if ($module == "hotels") {
            ...
            $data = array('vid_approved' => '1');
            $this->db->where('vid_id', $id);
            $this->db->update('pt_tour_videos', $data);
        } elseif ($module == "cruises") {
            $data = array('vid_approved' => '1');
            $this->db->where('vid_id', $id);
    ...
    function reject_multiple_videos()
    {
        ...
        foreach ($vidlist as $vid) {
            if ($module == "hotels") {
                ...
                $this->db->select('vid_type,vid_link');
                $this->db->where('vid_id', $vid);
                $rs = $this->db->get('pt_tour_videos')->result();
                $vidtype = $rs[0]->vid_type;
                $vidlink = $rs[0]->vid_link;
                $this->tours_model->delete_vids($vidlink, $vidtype, $vid);
            } elseif ($module == "cruises") {
                $this->load->model("cruises/cruises_model");
                $this->db->select('vid_type,vid_link');
                $this->db->where('vid_id', $vid);
