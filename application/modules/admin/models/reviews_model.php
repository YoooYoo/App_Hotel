...
// add review by customer
    ...
    {
        $data = array('review_booking_id' => $this->input->post('bookingid'), 'review_user' => $this->input->post('userid'), 'review_name' => $this->input->post('fullname'), 'review_comment' => $this->input->post('reviews_comments'), 'review_module' => $this->input->post('reviewmodule'), 'review_itemid' => $this->input->post('reviewfor'), 'review_date' => time(), 'review_clean' => $this->input->post('reviews_clean'), 'review_staff' => $this->input->post('reviews_staff'), 'review_facilities' => $this->input->post('reviews_facilities'), 'review_location' => $this->input->post('reviews_location'), 'review_comfort' => $this->input->post('reviews_comfort'), 'review_overall' => $this->input->post('overall'), 'review_status' => $status);
        $this->db->insert('pt_reviews', $data);
    }
...
// add review by public
    function add_review_public($status)
    {
        ...
        // INSERT PT_CONTEXT_REVIEWS TABLE
        $context_reviews = $this->input->post('reviews_contexts');
        foreach ($context_reviews AS $key => $context_review) {
            $data_context_reviews = [
                'review_id' => $insert_id,
                'context_id' => $context_review
            ];

            $this->db->insert('pt_context_reviews', $data_context_reviews);
        }
    }
