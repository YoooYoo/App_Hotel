...
// update seo settings
		function update_seo_settings() {
				$data = array('seo_status' => $this->input->post('seo_status'), 'keywords' => $this->input->post('keywords'), 'meta_description' => $this->input->post('meta_description'), 'home_title' => $this->input->post('home_title'));
...
// update Js settings
		function update_js() {
				$data = array('google' => $this->input->post('gacode'), 'javascript' => $this->input->post('jcode'));
				$this->db->where('user', 'webadmin');
				$this->db->update('pt_app_settings', $data);
		}
...
// update general settings
		function update_settings() {
				...
				$data = array('offline_message' => $this->input->post('offlinemsg'),'rss' => $this->input->post('rss'),'default_theme' => $this->input->post('theme'),'site_offline' => $this->input->post('site_offine'), 'site_title' => $this->input->post('site_title'), 'site_url' => $this->input->post('site_url'), 'license_key' => $this->input->post('license'), 'site_name' => $this->input->post('site_name'), 'tag_line' => $this->input->post('tag_line'), 'address' => $this->input->post('address'), 'phone' => $this->input->post('phone'), 'copyright' => $this->input->post('copyright'), 'reviews' => $this->input->post('reviews'), 'gallery_approve' => $galleryApprove, 'video_uploads' => $this->input->post('videouploads'), 'default_lang' => $this->input->post('default_lang'), 'multi_lang' => $this->input->post('multi_lang'), 'default_city' => $this->input->post('default_city'), 'date_f' => $dateformat, 'date_f_js' => $jsdate, 'booking_expiry' => $expiry, 'booking_cancellation' => $cancel, 'coupon_code_length' => $codelength, 'coupon_code_type' => $this->input->post('coupon_code_type'), 'searchbox' => $this->input->post('searchbox'), 'allow_registration' => $this->input->post('allow_registration'), 'allow_supplier_registration' => $this->input->post('allow_supplier_registration'));
				$this->db->where('user', 'webadmin');
				$this->db->update('pt_app_settings', $data);
...
// update security settings
		function update_security() {
				$data = array('ssl_url' => $this->input->post('ssl_url'), 'secure_admin_key' => $this->input->post('secure_admin_key'), 'secure_admin_status' => $this->input->post('secure_admin_status'), 'secure_supplier_key' => $this->input->post('secure_supplier_key'), 'secure_supplier_status' => $this->input->post('secure_supplier_status'));
				$this->db->where('user', 'webadmin');
				$this->db->update('pt_app_settings', $data);
		}
...
//get business hours
		...
		public function get_business_hours() {
				return $this->db->get('pt_business_hours')->result();
		}

// Update business hours
		function update_business_hrs() {
				$bhtime = $this->input->post('bh_time');
				$bhtype = $this->input->post('day_type');
				foreach ($bhtime as $bhid => $bhval) {
						$data = array('bh_time' => $bhval);
						$this->db->where('bh_id', $bhid);
						$this->db->update('pt_business_hours', $data);
				}
				foreach ($bhtype as $btid => $btval) {
						$data = array('bh_type' => $btval);
						$this->db->where('bh_id', $btid);
						$this->db->update('pt_business_hours', $data);
				}
		}
