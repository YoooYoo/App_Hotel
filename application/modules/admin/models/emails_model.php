...
// Call the Model constructor
				...
				if ($mailsettings[0]->mail_default == "smtp") {
					...
					if($mailsettings[0]->mail_secure == "no"){
						...
						$config = Array('protocol' => 'smtp', 'smtp_host' => $secure.$mailsettings[0]->mail_hostname, 'smtp_port' => $mailsettings[0]->mail_port, 'smtp_user' => $mailsettings[0]->mail_username, 'smtp_pass' => $mailsettings[0]->mail_password, 'mailtype' => 'html', 'charset' => 'iso-8859-1', 'wordwrap' => TRUE,'smtp_auth' => TRUE);
...
//send email to customer
		function sendEmail_customer($details, $sitetitle) {
			...
			$currencysign = $details->currSymbol;
				...
				$values = array($invoiceid, $refno, $paymethod, $deposit, $totalamount, $sendto, $custid, $country, $phone, $currencycode, $currencysign, $invoicelink, $sitetitle, $duedate, $name);
				$message = $this->mailHeader;
...
//send email to Admin
		function sendEmail_admin($details, $sitetitle) {
			 ...
			 $currencysign = $details->currSymbol;
				...
				$values = array($invoiceid, $refno, $paymethod, $deposit, $totalamount, $custemail, $custid, $country, $phone, $currencycode, $currencysign, $invoicelink, $sitetitle, $name);
...
//send email to Owner
		function sendEmail_owner($details, $sitetitle) {
			 ...
			 $custid = $details->bookingUser;
				...
				$message .= "Date :" . pt_show_date_php($date) . ".<br>";
...
//send email to customer for booking payment success
		function paid_sendEmail_customer($details) {
			...
			$currencysign = $details->currSymbol;
				...
				$values = array($invoiceid, $refno, $paymethod, $deposit, $totalamount, $sendto, $custid, $country, $phone, $currencycode, $currencysign, $invoicelink, $sitetitle, $remaining, $name);
...
//send email to Owner for booking paid
		function paid_sendEmail_owner($details, $sitetitle, $currencysign, $currencycode) {
				$currencycode = $details->currCode;
				$currencysign = $details->currSymbol;
...
// send Verification Email
		...
		public function email_verified($to, $pass, $name, $phone) {
...
// send New supplier signup email to Admin
		...
		public function shortcode_variables($template) {
				if ($template == "bookingorderadmin") {
						$result = array("{invoice_id}", "{invoice_code}", "{payment_method}", "{deposit_amount}", "{total_amount}", "{customer_email}", "{customer_id}", "{country}", "{phone}", "{currency_code}", "{currency_sign}", "{invoice_link}", "{site_title}", "{fullname}");
				}
				...
				elseif ($template == "bookingordercustomer") {
						$result = array("{invoice_id}", "{invoice_code}", "{payment_method}", "{deposit_amount}", "{total_amount}", "{customer_email}", "{customer_id}", "{country}", "{phone}", "{currency_code}", "{currency_sign}", "{invoice_link}", "{site_title}", "{due_date}", "{fullname}");
				}
				elseif ($template == "bookingpaidcustomer") {
						$result = array("{invoice_id}", "{invoice_code}", "{payment_method}", "{deposit_amount}", "{total_amount}", "{customer_email}", "{customer_id}", "{country}", "{phone}", "{currency_code}", "{currency_sign}", "{invoice_link}", "{site_title}", "{remaining_amount}", "{fullname}");
				}
