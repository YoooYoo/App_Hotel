...
//search hotels by text
                                   ...
                                   $lists = null, $checkin = null, $checkout = null)
    {
        ...
        if (!empty ($sprice)) {
            ...
            $minp = $sprice[0];
            $maxp = $sprice[1];
            $this->db->having('pt_hotels.hotel_basic_price >=', $minp);
            $this->db->having('pt_hotels.hotel_basic_price <=', $maxp);
        }
...
// Get hotel settings data for adding hotel or room
    ...
    function updateHotelSettings()
    {
        ...
        $data = array('front_icon' => $this->input->post('page_icon'),
            ...
            'front_txtsearch' => '1',
            'front_tax_percentage' => $this->input->post('taxpercentage'), 'front_tax_fixed' => $this->input->post('taxfixed'), 'front_search_state' => $this->input->post('state'), 'front_sharing' => $this->input->post('sharing'), 'linktarget' => $this->input->post('target'), 'header_title' => $this->input->post('headertitle'));
