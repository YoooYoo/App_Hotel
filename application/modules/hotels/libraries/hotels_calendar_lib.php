...
class Hotels_calendar_lib extends CI_Calendar
{
	...
	{
		return  array (
						'table_open'				=> '<table style="margin-left:3%;height: 241px;" class="table-condensed panel col-md-3">',
						'heading_row_start'			=> '<tr>',
						'heading_previous_cell'		=> '<th><a href="{previous_url}">&lt;&lt;</a></th>',
						'heading_title_cell'		=> '<th colspan="{colspan}" style="font-weight:300" class="text-center">{heading}</th>',
						'heading_next_cell'			=> '<th><a href="{next_url}">&gt;&gt;</a></th>',
						'heading_row_end'			=> '</tr>',
...
/**** functions for front ****/
			...
			for ($i = 0; $i < 7; $i++)
			{
                ...
                if(!$rchk){
                 // if($past > time()){
                  ...
                  // }else{
                  //    $out .= ($day > 0 AND $day <= $total_days) ? $this->temp['cal_cell_start_past'] : $this->temp['cal_cell_start'];
                  // }
 ...
 $totalquantity = $run[0]->room_quantity;
  ...
  return calendar_room_check($roomid,$totalquantity,$chkd);
