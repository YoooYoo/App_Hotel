...
//set hotel id by id
    ...
    function search_hotels($offset = null)
    {
        ...
        // $hotelslist = $this->hotelswithrooms();
        $rh = $this->ci->hotels_model->search_hotels_front('', '', '', '', $hotelslist);
        $hotels = $this->ci->hotels_model->search_hotels_front($perpage, $offset, $orderby);
        $data['all_hotels'] = $this->getResultObject($hotels['all']);
        $data['paginationinfo'] = array('base' => 'hotels/search', 'totalrows' => $rh['rows'], 'perpage' => $perpage);

        $data['plinks'] = $this->ci->bootpagination->dopagination('hotels/search', $rh['rows'], $perpage);
        return $data;
    }

    function search_hotels_by_text($offset = null, $checkin = null, $checkout = null)
    {
        $this->ci->load->library('bootpagination');
        $data = array();
        $settings = $this->settings();
        ...
        $hotels = $this->ci->hotels_model->search_hotels_by_text($perpage, $offset, $orderby, $checkin, $checkout);

        $rh = $this->ci->hotels_model->search_hotels_by_text('', '', '', '');
...
// get hotel rooms with limited details
    ...
    function hotel_details()
    {
        ...
        $detailResults = (object)array('id' => $details[0]->hotel_id, 'title' => $title, 'slug' => $slug, 'thumbnail' => $thumbnail,
            'stars' => pt_create_stars($stars), 'starsCount' => $stars, 'location' => $city, 'desc' => $desc, 'amenities' => $amenities,
   ...
   hotel_trusted,hotel_best_price,hotel_stars,hotel_slug,hotel_refundable,hotel_ratings,hotel_arrivalpay,thumbnail_image,hotel_amenities,hotel_latitude,hotel_longitude,hotel_meta_keywords,hotel_meta_desc');
        ...
        $this->location = $details[0]->hotel_city;
...
// hotel Reviews
    function hotelReviews()
    {
        ...
        $this->db->where('review_itemid', $this->hotelid);
        $this->db->order_by('review_id', 'desc');
        return $this->db->get('pt_reviews')->result();
    }

// hotel Reviews for API
    function hotel_reviews_for_api()
    {
        $result = array();
...
// hotel Reviews Averages
    ...
    {
        $this->db->select('room_id,room_quantity,room_title,room_desc,room_adults,room_children,room_amenities,thumbnail_image,extra_bed,extra_bed_charges');
...
// List 2 top rated hotels from each city passed to to this function
    ...
    function getLimitedResultObject($hotels)
    {
        ...
        foreach ($hotels as $h) {
            ...
            $price = $curr->convertPrice($bestprice, 0);
            $result[] = (object)array('id' => $this->hotelid, 'title' => $this->title, 'slug' => base_url() . 'hotels/' . $this->slug, 'thumbnail' => $this->thumbnail,
                'stars' => pt_create_stars($this->stars), 'location' => $this->location, 'price' => $price, 'currCode' => $curr->code, 'currSymbol' => $curr->symbol);
        }
        return $result;
    }
    ...
    function getRoomsResultObject($rooms, $checkin = null, $checkout = null)
    {
        ...
        foreach ($rooms as $room) {
            ...
            $bedcharges = $curr->convertPrice($roomprice['extrabed'], 0);
    ...
    function getBookResultObject($hotelid, $roomid, $roomscount, $extrabeds = 0, $checkin = null, $checkout = null)
    {
        ...
        $price = $curr->convertPrice($totalPrice, 0);
    ...
    function getUpdatedDataBookResultObject($hotelid, $roomid, $checkin, $checkout, $roomscount, $extras, $extrabeds = 0)
    {
        ...
        foreach ($extratotal['extrasInfo'] as $einfo) {
            $extrasHtml .= "<span class='pull-left go-right'>" . $einfo['title'] . "</span>
          					<span class='pull-right strong go-left'>" . $curr->code . " " . $curr->symbol . $curr->convertPrice($einfo['price'], 0) . "</span>
          					<div class='clearfix'></div><div class='line'></div>";
        }

        $subitem = array("id" => $roomid, "price" => $curr->convertPriceFloat($perNight), "count" => $roomscount);
